...
namespace Telia.MVC.Areas.Identity.Pages.Account
{
    [AllowAnonymous]
    public class RegisterModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                var user = new User
                {
                    UserName = Input.Email, Email = Input.Email, Name = Input.Name, LastName = Input.LastName,
        ...
        {
            bool x = await _roleManager.RoleExistsAsync("Admin"); //tikriname ar yra tokia rolė
            if (!x)
            {
                //Sukuriame tokią rolę
                await _roleManager.CreateAsync(new IdentityRole() {Name = "Admin"});
                //createing remainig roles

                x = await _roleManager.RoleExistsAsync("Moderator");
